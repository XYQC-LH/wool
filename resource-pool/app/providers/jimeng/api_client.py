...
class ApiClient:
    ...
    def generate_t2i(self, prompt: str, model: str, ratio: str, seed: int = -1):
        ...
        try:
            ...
            required_credit = estimate_required_credit(model, images_per_request=DEFAULT_IMAGES_PER_REQUEST)
            if required_credit is not None:
                if not self.token_manager.find_account_with_sufficient_credit(required_credit):
                    return {
                        "error": True,
                        "response": f"所有账号积分均不足{required_credit}点（模型{model}，{DEFAULT_IMAGES_PER_REQUEST}张/次），无法生成。",
                    }
    ...
    def submit_t2i(self, prompt: str, model: str, ratio: str, seed: int = -1) -> Dict[str, Any]:
        """仅提交文生图任务（不在此处阻塞轮询）
        Returns:
            dict: 包含 history_id/history_record_id、submit_id、可选 queue_message
        """
        # 获取实际的模型key
        model = self._get_model_key(model)

        required_credit = estimate_required_credit(model, images_per_request=DEFAULT_IMAGES_PER_REQUEST)
        if required_credit is not None and self.token_manager.get_account_count() > 1:
            if not self.token_manager.find_account_with_sufficient_credit(required_credit):
                return {
                    "error": True,
                    "response": f"所有账号积分均不足{required_credit}点（模型{model}，{DEFAULT_IMAGES_PER_REQUEST}张/次），无法生成。",
        ...
        # 生成随机种子（保持与原逻辑一致）
        seed = random.randint(1, 999999999) if seed == -1 else seed
        ...
        data = {
            "extend": {"root_model": model_req_key, "template_id": ""},
            "submit_id": submit_id,
            "metrics_extra": json.dumps(metrics_extra),
            "draft_content": json.dumps(
                {
        ...
        response = self._send_request("POST", url, params=params, json=data)
        if not response or str(response.get("ret")) != "0":
            logger.error(f"[Jimeng] Failed to submit t2i: {response}")
            return {"error": True, "response": response}

        history_id = response.get("data", {}).get("aigc_data", {}).get("history_record_id")
        if not history_id:
            logger.error("[Jimeng] No history_id in submit t2i response")
            return {"error": True, "response": response}

        result: Dict[str, Any] = {"submit_id": submit_id, "history_id": history_id, "history_record_id": history_id}
        try:
            queue_info = self._get_queue_info_from_response(history_id)
            if queue_info:
                result["is_queued"] = True
                result["queue_message"] = self._format_queue_message(queue_info)
        except Exception:
            pass

        return result

    def _fetch_history_record(
        self,
        *,
        history_id: Optional[str] = None,
        submit_id: Optional[str] = None,
    ) -> Optional[Dict[str, Any]]:
        """拉取 get_history_by_ids 的单条记录（用于增量轮询）"""
        if not history_id and not submit_id:
            return None
        ...
        history_data = data_map.get(key)
        if not history_data and isinstance(data_map, dict) and len(data_map) == 1:
            history_data = next(iter(data_map.values()))
        if not history_data and submit_id and isinstance(data_map, dict):
            for _k, _v in data_map.items():
                if isinstance(_v, dict) and _v.get("submit_id") == submit_id:
                    history_data = _v
                    break
        return history_data

    def _extract_image_urls_from_history_data(
        self,
        history_data: Dict[str, Any],
        *,
        filter_uploaded_images: bool,
    ) -> List[str]:
        image_urls: List[str] = []
        ...
        for item in item_list:
            ...
            if image and "large_images" in image:
                for large_image in image.get("large_images", []) or []:
        ...
        if filter_uploaded_images:
            try:
                draft_content_str = history_data.get("draft_content", "") or ""
                if draft_content_str:
                    draft_content_dict = json.loads(draft_content_str)
                    component_list = draft_content_dict.get("component_list", []) or []
                    for component in component_list:
                        abilities = component.get("abilities", {}) or {}
                        blend_data = abilities.get("blend", {}) or {}
                        ability_list = blend_data.get("ability_list", []) or []
                        for ability in ability_list:
                            if ability.get("name") == "byte_edit":
                                for _uri in ability.get("image_uri_list", []) or []:
                                    upload_image_uris.add(_uri)
            except Exception as e:
                logger.debug(f"[Jimeng] 解析 draft_content 失败（忽略，不影响结果提取）: {e}")

        resources = history_data.get("resources", []) or []
        for resource in resources:
            if resource.get("type") != "image":
                continue
            resource_uri = resource.get("key") or resource.get("uri") or ""
            if filter_uploaded_images and upload_image_uris and resource_uri and resource_uri in upload_image_uris:
                continue
            image_info = resource.get("image_info", {}) or {}
            u = image_info.get("image_url")
            if u:
                image_urls.append(u)
    ...
    def _extract_progress_info(self, history_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key in (
            ...
            percent = self._coerce_percent(history_data.get(key))
        ...
        for container_key in (
            ...
            if percent is not None:
                break
            container = history_data.get(container_key)
            if not isinstance(container, dict):
                continue
            for key in ("progress_percent", "progress", "percent", "pct", "ratio", "rate", "schedule"):
                percent = self._coerce_percent(container.get(key))
                if percent is not None:
                    break
            for key in ("status_msg", "status_message", "message", "msg", "text", "stage", "phase"):
                v = container.get(key)
        ...
        return {"progress_percent": percent, "progress_text": text}
    ...
    def get_generation_snapshot(
        ...
        *,
        history_id: Optional[str] = None,
        submit_id: Optional[str] = None,
        filter_uploaded_images: bool = False,
    ) -> Optional[Dict[str, Any]]:
        """获取一次生成快照（用于增量下载）"""
        history_data = self._fetch_history_record(history_id=history_id, submit_id=submit_id)
        if not history_data:
            return None
        ...
        result = {
            ...
            "image_urls": self._extract_image_urls_from_history_data(
                history_data, filter_uploaded_images=filter_uploaded_images
            ),
        }
        result.update(self._extract_progress_info(history_data))
        return result

    def generate_i2i(self, image: Optional[Any] = None, images: Optional[List[Any]] = None, prompt: str = "", model: str = "3.0", ratio: str = "1:1", seed: int = -1, num_images: int = 4) -> Tuple[ImageTensor, str, str]:
        """处理图生图请求"""
        try:
            ...
            required_credit = estimate_required_credit(model_key, images_per_request=DEFAULT_IMAGES_PER_REQUEST)
            if required_credit is not None:
                if not self.token_manager.find_account_with_sufficient_credit(required_credit):
                    return self._create_error_result(
                        f"所有账号积分均不足{required_credit}点（模型{model_key}，{DEFAULT_IMAGES_PER_REQUEST}张/次），无法生成。"
                    )
            ...
            if result.get("is_queued"):
                ...
                for attempt in range(max_retries):
                    ...
                    if image_urls:
                        ...
                        image_batch = np.concatenate(images, axis=0)
                        generation_info = self._generate_info_text(prompt, model_key, ratio, len(images))
    ...
    def _get_generated_images_by_history_id(self, history_id):
        ...
        try:
            ...
            # 1) 优先从 item_list 提取（官网最终结果的权威来源）
            item_list = history_data.get("item_list", []) or []
            for item in item_list:
                image = item.get("image", {}) or {}
                if image and "large_images" in image:
            ...
            # 2) 回退从 resources 提取，并尽量过滤掉参考图
            resources = history_data.get("resources", []) or []
            draft_content = history_data.get("draft_content", "") or ""
            upload_image_uris = set()
            try:
                if draft_content:
                ...
                logger.debug(f"[Jimeng] 解析draft_content失败（忽略，不影响结果提取）: {e}")
            ...
            for resource in resources:
                ...
                image_info = resource.get("image_info", {}) or {}
                resource_uri = resource.get("key") or resource.get("uri") or ""
                if upload_image_uris and resource_uri and resource_uri in upload_image_uris:
                    continue
                image_url = image_info.get("image_url")
                if image_url:
                    image_urls.append(image_url)
    ...
    def _get_generated_images_by_submit_id(self, submit_id: str):
        ...
        try:
            ...
            data_map = result.get("data", {}) or {}
            history_data = data_map.get(submit_id)
            if not history_data and isinstance(data_map, dict) and len(data_map) == 1:
                history_data = next(iter(data_map.values()))
            if not history_data and isinstance(data_map, dict):
                for _k, _v in data_map.items():
                    if isinstance(_v, dict) and _v.get("submit_id") == submit_id:
                        history_data = _v
                        break
    ...
    def _create_error_result(self, error_msg: str) -> Tuple[ImageTensor, str, str]:
        """创建错误结果
        Args:
            error_msg: 错误信息
        Returns:
            Tuple[ImageTensor, str, str]: (错误图像, 错误信息, 空URL列表)
        """
        logger.error(f"[Jimeng] {error_msg}")
        error_image = np.ones((1, 256, 256, 3), dtype=np.float32) * np.asarray([1.0, 0.0, 0.0], dtype=np.float32)
        return (error_image, f"错误: {error_msg}", "")

    def _download_images(self, urls: List[str], target_size: Optional[Tuple[int, int]] = None) -> List[ImageTensor]:
        """下载图片并转换为统一尺寸的张量
        Args:
            urls: 图片URL列表
            target_size: 期望的 (width, height)，用于统一所有图片尺寸；为 None 时不缩放
        Returns:
            List[ImageTensor]: 图片张量列表 (形状: [1, H, W, 3])
        """
        images = []
        for url in urls:
            try:
                ...
                # 期望形状为 [1, H, W, 3]
                images.append(np.expand_dims(np_image, axis=0))
            except Exception as e:
                logger.error(f"[Jimeng] 下载或处理图片失败 {url}: {e}")
                continue
        return images

    def _save_input_image(self, image_tensor: Any) -> Optional[str]:
        """将输入的图像张量保存为临时文件
        Args:
            image_tensor: 输入图像张量
        Returns:
        ...
        try:
            ...
            # 将输入转换为 numpy.ndarray（不依赖 torch）
            if hasattr(image_tensor, "detach") and hasattr(image_tensor, "cpu") and hasattr(image_tensor, "numpy"):
                image_tensor = image_tensor.detach().cpu().numpy()
            elif isinstance(image_tensor, Image.Image):
                image_tensor = np.array(image_tensor.convert("RGB"))
            else:
                image_tensor = np.asarray(image_tensor)

            if image_tensor.ndim == 4:  # batch, height, width, channels
            ...
            if image_tensor.ndim == 3 and image_tensor.shape[-1] == 4:
                image_tensor = image_tensor[..., :3]
            if image_tensor.ndim == 2:
                image_tensor = np.stack([image_tensor] * 3, axis=-1)

            if np.issubdtype(image_tensor.dtype, np.integer):
                image_tensor = image_tensor.astype(np.float32) / 255.0
            else:
                image_tensor = image_tensor.astype(np.float32)
                if image_tensor.size and float(image_tensor.max()) > 1.5:
